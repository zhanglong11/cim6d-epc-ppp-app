...
export default {
  ...
  watch: {
    dataAs: {
      handler(val) {
        if (histogramuCharts[this.canvasId]) {
          ...
          const maxValue = _.chain(val[this.valueKey]).map('data').flatten().max().value()
          if (maxValue <= 5) {
            histogramuCharts[this.canvasId].updateData({
              yAxis: {
  ...
  mounted() {
    this.render()
    const maxValue = _.chain(this.dataAs[this.valueKey]).map('data').flatten().max().value()
    if (maxValue <= 5) {
      histogramuCharts[this.canvasId].updateData({
        yAxis: {
          splitNumber: maxValue
  ...
  methods: {
    render() {
      let defaultOption = {
        ...
        padding: [15, 15, 15, 15], //画布填充边距，顺序为上右下左，同css，但必须4位
    ...
    touchstart(e) {
      ...
      histogramuCharts[this.canvasId].showToolTip(e, {
        ...
        format: (item, category) => {
          if (!this.showTooltipCategory) category = ''
          let defaultSetting = { type: 'number', fixed: 0, name: '' }
          let newName
          let { type, fixed, name } =
          ...
          if (typeof item.data === 'object') {
            if (type === 'number') {
              return `${category} ${item.name}:${_.round(item.data.value, fixed)}${name}`
            } else if (type === 'percent') {
              newName = name || '%'
              return `${category} ${item.name}:${_.round(item.data.value * 100, fixed)}${newName}`
            } else {
              return `${category} ${item.name}:${item.data.value}`
            }
          } else {
            if (type === 'number') {
              return `${category} ${item.name}:${_.round(item.data, fixed)}${name}`
            } else if (type === 'percent') {
              newName = name || '%'
              return `${category} ${item.name}:${_.round(item.data * 100, fixed)}${newName}`
            } else {
