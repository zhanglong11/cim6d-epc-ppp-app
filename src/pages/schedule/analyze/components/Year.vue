<template>
  <div>
    ...
    <uni-card v-for="company in tableData2" :key="company.title" :title="company.title">
      <t-table>
        <t-tr>
          <t-th></t-th>
          <t-th>进度领取数量</t-th>
        ...
        <t-tr v-for="(item, index) in company.data" :key="index">
          <t-td>{{ index + 1 + '月' }}</t-td>
          <t-td>{{ _.get(item, 'normalFinishedNum') }}</t-td>
          <t-td>{{ _.get(item, 'advanceFinishedNum') }}</t-td>
          <t-td>{{ _.get(item, 'delayFinishedNum') }}</t-td>
        ...
        .then(res => {
          ...
          this.chartData = {
            ...
            value: [
              {
                name: '正常完成数量',
                data: _.map(arr, e => e?.normalFinishedNum || 0)
              },
              {
                name: '超前完成数量',
                data: _.map(arr, e => e?.advanceFinishedNum || 0)
              },
              {
                name: '滞后延期数量',
                data: _.map(arr, e => e?.delayFinishedNum || 0)
              }
            ]
          }
        })
        ...
        .then(res => {
          ...
          this.chartData2 = {
            ...
            value: [
              {
                name: '正常完成数量',
                data: _.map(prettyDataArray, 'totalNormalFinishedNum')
              },
              {
                name: '超前完成数量',
                data: _.map(prettyDataArray, 'totalAdvanceFinishedNum')
              },
              {
                name: '滞后延期数量',
                data: _.map(prettyDataArray, 'totalDelayFinishedNum')
              }
            ]
          }
