<template>
  <div>
    <div v-if="chartData.label.length">
      <uni-card :title="`${year}年${month + 1}月进度分析柱状图`">
        <HistogramChart
          key="month-chart-1"
          labelKey="label"
          valueKey="value"
          ...
          :width="690"
          :height="600"
          style="width: 700rpx; height: 620rpx; padding: 15px 0; background-color: #fff"
        ></HistogramChart>
      </uni-card>
      <uni-card :title="`${year}年${month + 1}月不同状态任务完成数量占比统计`">
        <PieChart
          key="month-chart-2"
          :dataAs="chartData2"
          :colors="colors"
          ...
          :width="690"
          :height="600"
          style="width: 700rpx; height: 650rpx; padding: 15px 0; background-color: #fff"
        ></PieChart>
      </uni-card>
      <uni-card-tree :list="list" itemKey="taskId" style="margin-top: 15px">
        <template v-slot="{ item }">
          <div
            ...
            @click="$utils.toUrl('/pages/epcPpp/schedule/view/taskDetail?id=' + item.taskId)"
          >
