  ...
  <view class="main">
    ...
    <uni-drawer-custom ref="filterRef" @submit="handleDrawerFormChange" @reset="handleDrawerReset">
      <div>
        ...
        <uni-checkbox-button-group v-model="drawerForm.issueType" hasAll :options="safetyQuestionTypeList" />
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$utils.toUrl('/pages/epcPpp/safety/add')
    },
    handleItemClick(row) {
      if (row.status === 0) {
        //未取消
