        ...
        <block>
          <uni-card v-for="item in list" :key="item.id" @click.native="handleItemClick(item)">
            <view class="card-body">
              ...
              >
                <view class="title">施工周报{{ item.periodName}}</view>
              </view>
...
// #endif
export default {
  name: 'EpcPPPConstructionWeekLog',
  components: {
    Calendar
  },
  data() {
  ...
  methods: {
    ...
    //添加日志
    handleAdd() {
      this.$utils.toUrl('/pages/epcPpp/informationManage/constructionWeekLog/add')
    },
    handleItemClick(row) {
        //未取消
        this.$utils.toUrl(`/pages/epcPpp/informationManage/constructionWeekLog/detail?id=${row.id}`)
    },
