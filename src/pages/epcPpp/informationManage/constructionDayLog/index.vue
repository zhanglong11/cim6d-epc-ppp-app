        ...
        <block>
          <uni-card v-for="item in list" :key="item.id" @click.native="handleItemClick(item)">
            <view class="card-body">
              ...
              >
                <view class="title">施工日志{{ moment(item.reportDate).format('YYYY年MM月DD日') }}</view>
              </view>
...
export default {
  ...
  methods: {
    ...
    async getThisMonthLogList(){
      ...
      let currentData = _.map(datas, item => moment(item.reportDate,['YYYYMMDD']).format('YYYY-MM-DD')) //此次请求返回的含有日志项信息的日期
    ...
    //添加日志
    handleAdd() {
      this.$utils.toUrl('/pages/epcPpp/informationManage/constructionDayLog/add')
    },
    handleItemClick(row) {
        //未取消
        this.$utils.toUrl(`/pages/epcPpp/informationManage/constructionDayLog/detail?id=${row.id}`)
    },
