<template>
  <view>
    <h4 style="margin: 10rpx 0 10rpx 20rpx;">分析类型</h4>
    <HistogramChart
      :dataAs="IssueOptions"
      :xAxisAs="{ itemCount: 6 }"
      :yAxisAs="{ formatter: { unit: '' } }"
      :legendAs="legendAs"
      canvasId="qualityIssueMonthChart"
    ></HistogramChart>
    <h4 style="margin: 30rpx 0 0 20rpx;">{{ year }}年{{ month }}月不同种类的质量问题数量占比统计</h4>
    <view class="marginTop20">
      <PieChart
        ...
        canvasId="qualityIssueMonthTypeChart"
      />
    </view>
    <view class="marginTop20">
      <uni-search-bar
...
export default {
  ...
  computed: {
    ...
    qualityQuestionTypeList() {
      return this.$getArgList('epcPPPQualityIssueType') || []
    }
  },
  created() {
    this.getData()
