<template>
  <view>
    ...
    <h4 style="margin: 30rpx 0 0 20rpx;">{{ thisYearLabel[0] }}年质量问题数据对比分析表</h4>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      let thisYearInit = {
        year: thisYear?.[0]?.year || this.moment().format('YYYY'),
      ...
      let lastYearInit = {
        year: lastYear?.[0]?.year || this.moment().format('YYYY') - 1,
