  ...
  <view class="main">
    <div class="main-body">
      <uni-form-custom ref="form" :model="form" :rules="rules">
        <uni-card isForm>
          <!-- 点位选择组件 start -->
          <uni-select-anchor :uUid="uUid" isAddAnchor isEidt @change="changeAnchor" />
          <!-- 点位选择组件 start -->
          ...
          <uni-form-item label="问题类型" prop="issueType" isLink>
            <uni-select v-model="form.issueType" :options="qualityQuestionTypeList" placeholder="请选择问题类型" />
          </uni-form-item>
...
export default {
  ...
  computed: {
    ...
    qualityQuestionTypeList() {
      return this.$getArgList(this.form.engineeringCategory + '_qualityIssueType')
    },
    // 选择的构件
    selectGuid() {
